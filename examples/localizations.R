...
# Get the type object.
loc_type <- tator_api$GetLocalizationType(opt$type_id)
...
created_ids <- chunked_create(tator_api$CreateLocalizationList, project, localizations)
...
# of IDs.
localizations <- tator_api$GetLocalizationList(project, media.id = opt$video_id)
...
localizations <- tator_api$GetLocalizationList(project, media.id = opt$video_id, search = "_x:<0.5")
...
localizations <- tator_api$GetLocalizationList(project, media.id = opt$video_id, search = "_width:<0.25")
...
localizations <- tator_api$GetLocalizationList(project, media.id = opt$video_id, search = "_frame:>=100 AND _frame:<=200")
loginfo(paste("Found", length(localizations), "localizations between frames 100-200!"))

# Delete localizations between frames 100-200.
response <- tator_api$DeleteLocalizationList(project, media.id = opt$video_id, search = "_frame:>=100 AND _frame:<=200")
...
for (loc in localizations[1:10]) {
  ...
  response <- tator_api$UpdateLocalization(loc$id, localization.update = update)
  loginfo(response$message)
}